...
Metric.prototype = {
  ...
  // send metric data to the client
  runner: function(metric) {
    ...
    metric.socket.broadcast(JSON.stringify(metric.data));
  ...
  minuteRunner: function(metric) {
	...
	// increment the minute, hour, and day data
	...
	var timestampHour = null;
	metric.minuteData.events.forEach(function(event) {
		event.timestamp.setMilliseconds(0);
		event.timestamp.setSeconds(0);
		event.timestamp.setMinutes(0);
		metric.collection.update({ hour: event.timestamp, ext3: event.ext3, ext4: event.ext4, ext5: event.ext5 },
								 { "$inc" : {total: 1} },
								 { upsert: true },
								 function(err, result) {}
								);
...
// determine the list of .js files in the metrics folder
...
// run the callback for each metric in the metricsPath
